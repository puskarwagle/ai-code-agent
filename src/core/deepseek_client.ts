...
Refine the plan based on this feedback. Output the updated plan in the same JSON format.`
  ...
  /**
   * Create a MICRO-plan (3-5 steps only) by analyzing raw HTML
  ...
  async createMicroPlan(
    ...
    const truncatedHtml = pageHtml.substring(0, 50000);

    const messages: DeepSeekMessage[] = [
      {
        role: 'system',
        content: `You analyze raw HTML and create MICRO-PLANS with 3-5 SMALL, TESTABLE steps.

CRITICAL RULES:
1. ONLY 3-5 steps max.
2. Each step must be ONE atomic action.
3. Each step must be TESTABLE.
4. Base your plan ONLY on the provided HTML. Find real, usable selectors. DO NOT HALLUCINATE.
5. Think about the NEXT logical action to move towards the goal.
...
OUTPUT FORMAT: JSON only
{
  ...
  "estimated_time": "30 seconds"
      ...
      },
      {
        ...
        content: `
OVERALL GOAL: ${goal}
${additionalContext ? `USER GUIDANCE: ${additionalContext}` : ''}
CURRENT URL: ${currentState}
...
Based on the HTML, create a MICRO-PLAN with 3-5 small steps to move closer to the goal.`
      }
    ];
    ...
    try {
      ...
      console.error('Failed to parse micro-plan from HTML:', e);
    }
  ...
  async isLoginRequired(pageHtml: string): Promise<boolean> {
    const truncatedHtml = pageHtml.substring(0, 75000);

    const messages: DeepSeekMessage[] = [
      {
        role: 'system',
        content: 'You are an expert at analyzing webpage HTML to determine if a user is logged in or if a login is required. Look for forms with password fields, "Sign In" or "Log In" buttons, or a lack of personalized content. Conversely, look for "Log Out", "My Account", or profile-related links. Answer ONLY "yes" (if login is required) or "no" (if the user appears to be logged in).'
      },
      {
        role: 'user',
        content: `
Based on the following HTML, is a login required, or does the user seem to be already logged in?
