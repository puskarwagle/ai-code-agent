...
import { UniversalOverlayV2 } from '../core/universal_overlay_v2';
...
export async function* openHomepage(ctx: WorkflowContext): AsyncGenerator<string, void, unknown> {
  try {
    ...
    if (ctx.driver) {
      ...
      if (!ctx.overlay) {
        ctx.overlay = new UniversalOverlayV2(ctx.driver, 'Seek');
      }

      // Initialize and show overlay BEFORE navigation
      await ctx.overlay.initialize();
      await ctx.overlay.showJobProgress(0, 0, 'homepage_opened', 1);
    ...
    ctx.overlay = new UniversalOverlayV2(driver, 'Seek');

    await StealthFeatures.hideWebDriver(driver);
    await StealthFeatures.randomizeUserAgent(driver);

    // Initialize overlay BEFORE navigating to website
    await ctx.overlay.initialize();
    await ctx.overlay.showJobProgress(0, 0, 'homepage_opened', 1);
