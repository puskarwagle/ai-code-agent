...
import { UniversalOverlayV2 } from './universal_overlay_v2';
...
export class WorkflowEngine {
  ...
  private overlay: UniversalOverlayV2 | null = null;
  ...
  async executeStep(stepName: string): Promise<string> {
    ...
    try {
      ...
      if (!activeOverlay && !this.overlay && this.context.driver) {
        ...
        this.overlay = new UniversalOverlayV2(this.context.driver, 'Workflow');
        try {
      ...
      if (overlayToUpdate && typeof overlayToUpdate.updateJobProgress === 'function') {
        try {
          // Pass yield event to overlay for human-friendly messaging
          const appliedJobs = this.context.applied_jobs || 0;
          const totalJobs = this.context.total_jobs || 0;

          await overlayToUpdate.updateJobProgress(
            appliedJobs,
            totalJobs,
            result,  // This is the yield event name
            stepConfig.step
          );
        } catch (error) {
